...
#
#     ALL_PATH = "/share/share/pre_img/LTIS/LTISEXTRACT_90"
#     imgs = []
...
#         shutil.copyfile(ct, 'pre_img/train/%03d.png' % i)
#         shutil.copyfile(mask, 'pre_img/train/%03d_mask.png' % i)
#
#     for i, (ct, mask) in enumerate(imgs[-20:]):
#         shutil.copyfile(ct, 'pre_img/val/%03d.png' % i)
#         shutil.copyfile(mask, 'pre_img/val/%03d_mask.png' % i)
#     return imgs


def make_dataset(img_root,mask_root):
    imgs=[]
    n=len(os.listdir(img_root))
    for i in range(n):
        img=os.path.join(img_root,"%d.png"%i)
        mask=os.path.join(mask_root,"%d.png"%i)
...
class LiverDataset(data.Dataset):
    def __init__(self, img_root,mask_root, transform=None, target_transform=None):
        imgs = make_dataset(img_root,mask_root)
        self.imgs = imgs
